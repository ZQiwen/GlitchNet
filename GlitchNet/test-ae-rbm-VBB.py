...
flags.DEFINE_integer('epochs_au', 150, 'The number of training epochs for the autoencoder')
flags.DEFINE_integer('batchsize', 50, 'The batch size')
flags.DEFINE_boolean('restore_rbm', True, 'Whether to restore the RBM weights or not.')

# ensure output dir exists
if not os.path.isdir('out'):
  os.mkdir('out')

mydataset = input_seismo.read_data_sets(FLAGS.data_dir)
...
# RBMs
rbmobject1 = RBM(600, 256, ['rbmw1', 'rbvb1', 'rbmhb1', 'rbmva1', 'rbmha1'])
...
# Autoencoder
autoencoder = AutoEncoder(600, [256, 128, 64, 32], [['rbmw1', 'rbmhb1', 'rbmha1'],
                                                     ['rbmw2', 'rbmhb2', 'rbmha2'],
                                                     ['rbmw3', 'rbmhb3', 'rbmha3'],
...
for i in range(FLAGS.epochs_au):
  ...
  print('%d\t %f',i,ArithmeticErrorcost)
...
if 1:
  ...
  selected=np.where(relative_E_error>0.04)[0]#[40:60]
  fig, ax = plt.subplots(figsize=(4,12))
  kk=0
  for itrace in selected:
    plt.plot(np.linspace(-5,25,600),teX[itrace,:]+kk*2,'k')
    plt.plot(np.linspace(-5,25,600),teX_rec[itrace,:]+kk*2,'r')
    kk+=1
  plt.show()
  ...
  sine_func=np.sin(np.linspace(-5,25,600)).reshape(1,600)
  square_func=np.ones(shape=(1,600))
  square_func[0,300:]=0
  triang_func=square_func-np.linspace(1,0,600)
  plt.scatter(autoencoder.transform(noise)[:, axis1], autoencoder.transform(noise)[:, axis2], alpha=0.1,color='k')
  plt.scatter(autoencoder.transform(sine_func)[:, axis1], autoencoder.transform(sine_func)[:, axis2], alpha=1,color='b')
  plt.scatter(autoencoder.transform(square_func)[:, axis1], autoencoder.transform(square_func)[:, axis2], alpha=1,color='g')
  plt.scatter(autoencoder.transform(triang_func)[:, axis1], autoencoder.transform(triang_func)[:, axis2], alpha=1,color='m')
  ...
  plt.plot(np.linspace(-5,25,600),sine_func[0,:],'b')
  plt.plot(np.linspace(-5,25,600),autoencoder.reconstruct(sine_func)[0,:],'r')
  plt.plot(np.linspace(-5,25,600),square_func[0,:]+2,'g')
  plt.plot(np.linspace(-5,25,600),autoencoder.reconstruct(square_func)[0,:]+2,'r')
  plt.plot(np.linspace(-5,25,600),triang_func[0,:]+4,'m')
  plt.plot(np.linspace(-5,25,600),autoencoder.reconstruct(triang_func)[0,:]+4,'r')
  plt.show()
