...
class AutoEncoder(object):
    def __init__(self, input_size, layer_sizes, layer_names, tied_weights=False, 
                 ...
                 transfer_function=logistic,sigma=5e-2): # tf.nn.sigmoid
        ...
        for i in range(len(layer_sizes)):
            ...
            # Initialize a to one
            a = tf.Variable(tf.ones([dim]), name=layer_names[i][2])

            # We are going to use tied-weights so store the W matrix for later reference.
            self.encoding_matrices.append(W)
            self.encoding_biases.append(b)
            self.encoding_amps.append(a)

            output = transfer_function(tf.multiply(a, tf.matmul(next_layer_input, W) + b))
        ...
        for i, dim in enumerate(layer_sizes[1:] + [int(self.x.get_shape()[1])]):
            ...
            a = tf.Variable(tf.ones([dim]))
            self.decoding_matrices.append(W)
            self.decoding_biases.append(b)
            self.decoding_amps.append(a)

            output = transfer_function(tf.multiply(a, tf.matmul(next_layer_input, W) + b + tf.random.normal(tf.shape(b), 0., sigma)))
            next_layer_input = output
    ...
    def load_rbm_weights(self, path, layer_names, layer):
        saver = tf.train.Saver({layer_names[0]: self.encoding_matrices[layer],
                                ...
                                layer_names[2]: self.encoding_amps[layer]})
